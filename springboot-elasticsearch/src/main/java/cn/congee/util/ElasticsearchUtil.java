package cn.congee.util;

import cn.congee.annotation.Document;
import cn.congee.annotation.EsId;
...
public class ElasticsearchUtil {
    ...
    public <T> boolean batchSaveOrUpdate(List<T> list) throws Exception {
        ...
        for(BulkItemResponse bulkItemResponse : bulkResponse){
            ...
            if(bulkItemResponse.isFailed()){
                log.error("util 返回错误{}",bulkItemResponse.getFailureMessage());
                return false;
            }
        }
        return true;
    ...
    public XContentBuilder generateBuilder(Class clazz) throws IOException {
        ...
        for (Field f : declaredFields) {
            if (f.isAnnotationPresent(cn.congee.annotation.Field.class)) {
                // 获取注解
                cn.congee.annotation.Field  declaredAnnotation = f.getDeclaredAnnotation(cn.congee.annotation.Field.class);
                ...
                if (declaredAnnotation.type() == FieldType.OBJECT) {
                    ...
                    // 遍历该对象中的所有属性
                    for (Field f2 : df2) {
                        if (f2.isAnnotationPresent(cn.congee.annotation.Field.class)) {
                            // 获取注解
                            cn.congee.annotation.Field declaredAnnotation2 = f2.getDeclaredAnnotation(cn.congee.annotation.Field.class);
